...
#define DYNARRAY_DEFINE(PREFIX, TYPE, INIT_CAPACITY)                                                                                                \
	...
	CORE_DYNARRAY_IMPL(                                                                                                                         \
		static bool PREFIX##_grow_impl(PREFIX##_DynArray* darr) {                                                                           \
			...
			size_t new_capacity;                                                                                                        \
			if (darr->capacity == 0) {                                                                                                  \
				new_capacity = CORE_DYNARRAY_DEFAULT_CAPACITY;                                                                      \
			} else {                                                                                                                    \
				if (CORE_DYNARRAY_GROWTH_RATE <= 1) return false;                                                                   \
				if (darr->capacity > ((size_t)-1) / CORE_DYNARRAY_GROWTH_RATE) return false;                                        \
